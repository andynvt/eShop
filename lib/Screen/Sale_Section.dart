...
import 'package:eshop/Helper/Slideanimation.dart';
                            ...
                            children: [
                              Row(
                                ...
                                children: <Widget>[
                                  Hero(
                                    tag:
                                        "$index${widget.section_model!.productList![index].id}",
                                    child: ClipRRect(
      ...
      if (_isNetworkAvail) {
        if (CUR_USERID != null) {
          ...
          if (from == 1) {
            ...
            db.insertCart(model.id!,
                model.prVarientList![model.selVarient!].id!, qty, context);
          } else {
            if (int.parse(qty) > int.parse(model.itemsCounter!.last)) {
              // qty = productList[index].minOrderQuntity.toString();

              setSnackbar("${getTranslated(context, 'MAXQTY')!} ${model.itemsCounter!.last}",context);
            } else {
                    ...
                    children: <Widget>[
                      Expanded(
                        ...
                        children: [
                          Hero(
                            tag:
                                "$index${widget.sectionList![widget.index!].productList![index].id}",
                            child: ClipRRect(
                              borderRadius: const BorderRadius.only(
